...
impl<'mir, 'ctx: 'mir> Reference<'mir, 'ctx> {
    ...
    pub fn set_mode(&mut self, new_mode: Mode) {
        ...
        debug_assert!(new_mode != Mode::Cloned || self.can_be_cloned(), "borrow checker internal error: this reference `{:?}` cannot be changed to `Cloned` since its previous mode `{:?}` is incompatible with `Cloned`", self.place(), self.mode());
    ...
    /// Can the mode of this reference be changed to [`Mode::Cloned`].
    pub fn can_be_cloned(&self) -> bool {
        if let Some(mode) = &self.mode {
            match **mode {
                Mode::Borrowed | Mode::Cloned | Mode::Moved => true,
                Mode::SBorrowed | Mode::SMutBorrowed | Mode::MutBorrowed => false,
            }
        } else {
            false
        }
